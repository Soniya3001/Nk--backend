...
app.get("/api/quote", async function(req, res) {
  try {
    ...
    if (!info) return res.status(400).json({ error: "Symbol not supported: " + sym + ". Try: RELIANCE, TCS, HDFCBANK, SBIN, INFY" });
    ...
    console.log("Quote response for", sym, ":", JSON.stringify(data).slice(0, 200));

    // Token expired - re-login and retry
    if (!data.status && (data.errorcode === "AG8001" || data.errorcode === "AB1010" || data.errorcode === "AB8050")) {
      console.log("Token expired, re-logging in...");
      jwtToken = null;
      var relogged = await login();
      if (!relogged) return res.status(401).json({ error: "Re-login failed." });
      // Retry quote
      r = await fetch(AO_URL + "/rest/secure/angelbroking/market/v1/quote/", {
        method: "POST",
        headers: {
          "Content-Type":     "application/json",
          "Accept":           "application/json",
    ...
    if (!data.status) return res.status(400).json({ error: data.message || "Quote failed: " + JSON.stringify(data) });
    var q = data.data && data.data.fetched && data.data.fetched[0];
    if (!q) return res.status(400).json({ error: "No data returned for " + sym });
